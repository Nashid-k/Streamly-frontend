...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <>
      <header>
        <div className={`navbar ${isScrolled ? 'scrolled' : ''}`} style={{ marginLeft: 0, transition: 'margin-left 0.3s ease', background: isScrolled ? (platform === 'nprime' ? 'rgba(15, 23, 30, 0.95)' : 'var(--bg-elevated)') : 'linear-gradient(to bottom, rgba(0,0,0,0.7) 0%, rgba(0,0,0,0) 100%)', boxShadow: isScrolled ? '0 4px 12px rgba(0,0,0,0.5)' : 'none', backdropFilter: isScrolled ? 'blur(10px)' : 'none', height: '68px', padding: '0 4%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'fixed', top: 0, right: 0, left: 0, zIndex: 90 }}>
